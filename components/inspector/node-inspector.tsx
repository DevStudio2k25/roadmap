...
import { 
  Flag, CheckSquare, GitBranch, BookOpen, FileText, Table,
  Link2, Plus, Trash2
} from 'lucide-react';

const nodeConfig = {
  milestone: { icon: Flag, color: 'blue', label: 'Milestone' },
  task: { icon: CheckSquare, color: 'green', label: 'Task' },
  decision: { icon: GitBranch, color: 'amber', label: 'Decision' },
  resource: { icon: BookOpen, color: 'purple', label: 'Resource' },
  content: { icon: FileText, color: 'orange', label: 'Content' },
  table: { icon: Table, color: 'indigo', label: 'Table' },
};
...
export function NodeInspector() {
  ...
  const nodeType = node.type as keyof typeof nodeConfig;
    ...
    <div className="w-80 bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 h-full overflow-y-auto">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">{config.label} Properties</h3>
        </div>
      </div>
